...
if __name__ == "__main__":
    ...
    try:
        ...
        pl_hook = PLProfilingHook(batch_size=batch_size, num_batches=8192)
        trainer_kwargs["callbacks"].append(pl_hook)

        if "plugins" not in trainer_kwargs:
            trainer_kwargs["plugins"] = []
        if config.get("cluster_type", None) == "BCP":
            trainer_kwargs["plugins"].append(TorchElasticEnvironment())
